"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AttributeValueDoesNotAllowUpdatedError = void 0;
const common_1 = require("@heronjs/common");
const eav_errors_1 = require("./eav.errors");
class AttributeValueDoesNotAllowUpdatedError extends common_1.RuntimeError {
    constructor(message) {
        super(eav_errors_1.ErrorNamespaces.EAV, eav_errors_1.ErrorCodes.ATTRIBUTE_VALUE_DOES_NOT_ALLOW_UPDATE, message !== null && message !== void 0 ? message : 'EAV attribute does not allow update');
    }
}
exports.AttributeValueDoesNotAllowUpdatedError = AttributeValueDoesNotAllowUpdatedError;
